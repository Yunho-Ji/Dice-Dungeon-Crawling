...
func initialize_game_scene(player: Character, enemy: Character, battle_mgr: Node, ui_mgr: UIManager, stage_hud: Control):
...
func handle_battle_end(win: bool):
	...
	if win:
		...
		if player_node:
			var new_hp = min(player_node.get_max_hp(), player_node.get_current_hp() + player_node.get_recovery_power())
			player_node.set_current_hp(new_hp)
			player_node.update_hp_label()
