...
func show_stats(character: Character):
	...
	print("StatusPopup received character:", character.name, " HP:", character.current_hp, " MP:", character.current_mp)
	# 이전 스탯 정보 삭제
	...
	for stat_key in STAT_NAMES.keys():
		...
		match stat_key:
			"hp":
				value_text = "%s/%s" % [character.current_hp, character.max_hp]
			"mp":
				value_text = "%s/%s" % [character.current_mp, character.max_mp]
			_:
				value_text = str(character.get(stat_key))
		print("Stat:", stat_key, " Value:", value_text)
		
		var label = Label.new()
		label.text = "%s: %s" % [stat_name, value_text]
		label.add_theme_color_override("font_color", Color.WHITE) # 폰트 색상을 흰색으로 설정
